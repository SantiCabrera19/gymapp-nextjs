...
import { getUserWorkoutHistory } from '@/lib/api/training'
      ...
      <div className="space-y-8">
                ...
                {selectedRoutine ? (
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-slate-500">
                      {selectedRoutine.exercises?.length || 0} ejercicios • ~
                      {selectedRoutine.estimated_duration_minutes} min
                    </p>
                  </div>
                ) : (
                  <p className="text-slate-400">
        ...
        {!historyLoading && workoutHistory.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card className="p-6 text-center">
              ...
              <div className="text-2xl font-bold text-white mb-1">{workoutHistory.length}</div>
            ...
            <Card className="p-6 text-center">
              ...
              <div className="text-2xl font-bold text-white mb-1">
                {workoutHistory.length > 0
                  ? formatDuration(
                      Math.round(
                        workoutHistory.reduce((acc, w) => acc + w.total_duration_seconds, 0) /
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            {workoutHistory.length > 0 && (
              <Button variant="outline" size="sm" onClick={() => router.push('/training/history')}>
                Ver todos
              </Button>
            )}
          </div>
                ...
                <Card key={workout.id} className="p-4 hover:bg-slate-800/50 transition-colors">
                  <div className="flex items-center justify-between">
                    <div className="space-y-1">
                      <h4 className="font-medium text-white">{workout.name || 'Entrenamiento'}</h4>
                      <div className="flex items-center gap-4 text-sm text-slate-400">
