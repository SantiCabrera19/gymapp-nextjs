...
export function LoadingErrorBoundary({
  ...
  if (hasTimedOut) {
    return (
      <Card
        className={cn('p-8 text-center border-status-warning/20 bg-status-warning/5', className)}
      >
        ...
        <h3 className="text-lg font-semibold text-white mb-2">Carga Lenta Detectada</h3>
        <p className="text-text-secondary mb-6 max-w-md mx-auto">
          La página está tardando más de lo esperado en cargar. Esto puede deberse a una conexión
          lenta o problemas temporales del servidor.
        </p>
  ...
  if (error) {
    return (
      <Card className={cn('p-8 text-center border-status-error/20 bg-status-error/5', className)}>
        ...
        <h3 className="text-lg font-semibold text-white mb-2">Error de Carga</h3>
        <p className="text-text-secondary mb-6 max-w-md mx-auto">{error}</p>
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          {onRetry && (
            <Button onClick={onRetry} className="bg-status-error hover:bg-status-error/80">
              <RefreshCw size={16} className="mr-2" />
              Reintentar
            </Button>
          )}
